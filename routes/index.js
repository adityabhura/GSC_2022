...
router.post('/hospitalRegister',(req,res)=>{
    ...
    if(password != confirmPassword){
        ...
        res.render('register',{ 'err':err, 'email':email, 'name':name, 'password':password, 'location':location});
    }
...
passport.use('hospital',new localStrategy({usernameField:'email'}, (email,password,done)=>{
...
router.get('/hospitalDashboard', ensureAuthenticated, (req,res)=>{
    ...
    // })
    hospital.findOne({_id:req.user._id}).populate({path:"doctors",model:Doctor}).exec(function(err,data){
        if(err) throw err;
        else{
...
router.post('/add/:id', ensureAuthenticated, async(req,res)=>{
    ...
    hospital.findById(req.params.id,(err,hospital)=>{
        if(err) throw err;
            ...
            Doctor.create({registration: reg,name:name,specialization:spec}, (err,data)=>{
                if(err) throw err;
                else{
                    console.log(data);
